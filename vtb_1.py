...
#Сообщество
token = "894510403b00eec841ce2a53fd8cc7c251c0a8f1cae39d0387bd5a869d8a40512dc22921e1e60d94c7dd7"
...
keyboardMain = {
    ...
    [get_button(label='Задания 📒',color="secondary"),get_button(label='Приложить достижение.', color="positive")],
...
def sendphoto(user_id,msg):
    ...
    vk.method("messages.send", {"peer_id": user_id, "random_id": random.randint(-203030, 2032303),"message": msg, "attachment": f'photo{c["owner_id"]}_{c["id"]}', "keyboard": keyboardMain})
    ...
    try:
        ...
        if messages["count"] >= 1:
            ...
            if id not in kash_ids_students: #Если мы не знаем чела в вк
                ...
                vk.method("messages.send", {"peer_id": id, "message": text, "random_id": random.randint(1, 2147483647), "keyboard": keyboardChoice})
                ...
                #СТУДЕНТ рабочая область
                    ...
                    if body.lower() in Vuz_list:
                        ...
                        vk.method("messages.send", {"peer_id": id, "message": text, "random_id": random.randint(1, 2147483647), "keyboard": keyboardExit})
                    ...
                    if body.lower() == "профиль 🙄":       #Профиль
                        ...
                        text = "Вас зовут {}\nВы учитесь в {}\nНомер вашего студенческого билета {}\nВы выполнили {} заданий\nВаш рейтинг {}".format(kash_person[id][1],kash_person[id][2],kash_person[id][3],xxx,str(kash_person[id][7])[:3])
                        ...
                        if str(kash_person[id][5]) == '0':
                            ...
                            vk.method("messages.send", {"peer_id": id, "message": text, "random_id": random.randint(1, 2147483647), 'keyboard': keyboardQ})
                        elif str(kash_person[id][5]) == '1':
                            text = "Для вас доступно 1 задание"
                            vk.method("messages.send", {"peer_id": id, "message": text, "random_id": random.randint(1, 2147483647), 'keyboard': keyboardOne})
                        elif str(kash_person[id][5]) == '2':
                            text = "Для вас доступно 1 задание"
                            vk.method("messages.send", {"peer_id": id, "message": text, "random_id": random.randint(1, 2147483647), 'keyboard': keyboardTwo})
                        elif str(kash_person[id][5]) == '21' or str(kash_person[id][5]) == '12':
                            text = "Для вас нет доступных заданий"
                            vk.method("messages.send", {"peer_id": id, "message": text, "random_id": random.randint(1, 2147483647), 'keyboard': keyboardDouble})
                        else:
                        ...
                        if str(kash_person[id][5]) == '1' or str(kash_person[id][5]) == '12' or str(kash_person[id][5]) == '21':
                            ...
                            vk.method("messages.send", {"peer_id": id, "message": text, "random_id": random.randint(1, 2147483647), 'keyboard': keyboardReady})
                        ...
                        text = "Отправьте название мероприятия, в котором вы учавствовали."
                        ahiv[id] = [id,"название","Роль в мероприятии (участник, волонтёр, медиа, организатор)","город, страна, международный)",0,"prof"]
                        print(ahiv)
                        ...
                        if kash_person[id][6] == '1':#Если делает 1 задание
                            ...
                            if a == 1: #Подписан
                                ...
                                if str(kash_person[id][5]) == '2':
                                    ...
                                    q.execute("UPDATE Main SET rating = {} Where id = {}".format(kash_person[id][7],id))    # POINT403
                                    connection.commit()
                                    vk.method("messages.send", {"peer_id": id, "message": text, "random_id": random.randint(1, 2147483647), 'keyboard': keyboardMain})
                            ...
                            a = vk_user.method("likes.isLiked", {'type': 'post','owner_id':-22749457, 'item_id': 24957, 'user_id': id})
...
Наша платформа помогает абитуриентам, студентам и деканатам находить общий язык, повышать прозрачность и эффективность академической лояльности, взаимовыгодно для всех сторон интегрируя банк ВТБ и его партнёров в активную жизнь университета и города
                    ...
                    vk.method("messages.send", {"peer_id": id, "message": text, "random_id": random.randint(1, 2147483647), 'keyboard':keyboardAhiv})
                elif kash_person[id][4] == 111:
                    kash_person[id][4] = 112
                    ahiv[id][2] = body.title()  #Роль
                    text = "Каким по масштабу было данное мероприятие?\nНажмите кнопку."
                    vk.method("messages.send", {"peer_id": id, "message": text, "random_id": random.randint(1, 2147483647), 'keyboard':keyboardAhivONE}) 
                    ...
                    if ahiv[id][2] == "Участник":
                        ...
                        vk.method("messages.send", {"peer_id": id, "message": text, "random_id": random.randint(1, 2147483647), 'keyboard':keyboardAhivTWO})  
                    ...
                    text = 'Прикрепите фотографию документа, доказывающего ваше участие в мероприятии "{}"'.format(ahiv[id][1])
                    vk.method("messages.send", {"peer_id": id, "message": text, "random_id": random.randint(1, 2147483647)}) 
                    
                elif kash_person[id][4] == 116:
                    try:
                        ahiv[id][5] = messages["items"][0]["last_message"]['attachments'][0]['photo']['sizes'][len(messages["items"][0]["last_message"]['attachments'][0]['photo']['sizes'])-1]['url']
                        text = "Ваши данные отправляютя на проверку"
                        kash_person[id][4] == 104
                        print(ahiv[id])
                        sq = 'INSERT INTO Achiv (id,name,status,type,place,url) VALUES ({},"{}","{}","{}","{}","{}")'.format(ahiv[id][0],ahiv[id][1],ahiv[id][2],ahiv[id][3],ahiv[id][4],ahiv[id][5])
                        print(sq)           # DB403
                        q.execute(sq)
                        connection.commit()
                        vk.method("messages.send", {"peer_id": id, "message": text, "random_id": random.randint(1, 2147483647), 'keyboard':keyboardMain})
